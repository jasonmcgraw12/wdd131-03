...
searchButton = document.querySelector("#magnifying")
searchBar = document.querySelector("#search-bar")

// initializer
function init(){
    recipe = recipes[Math.floor(Math.random() * recipes.length)]
    displayRecipies([recipe])
}

// displaying recipies
function displayRecipies(foundRecipies){
    const filteredRecipies = foundRecipies.sort(objectCompareFn)
    removeHTML()
    foundRecipies.forEach((recipe) => {
        newHTML = `
        <div class="bordered">
            <img class="recipe-picture" src=${recipe.image} alt=${recipe.name}>
            <div class="info">
                ${getButtons(recipe.tags)}
                <h2>${recipe.name}</h2>
                <div class="text-box">
                    <p role="img" aria-label="4 out of 5 stars">Rating: ${getRating(recipe.rating)}</p>
                </div>
                <div class="text-box">
...
function getButtons(tags) {
    console.log(tags)
    return tags.map(tag =>{
        return `<button aria-label="link to recipe">${tag}</button>`
    }).join(" ")
}
...
function addHTML(newHTML) {
    document.querySelector("#recipies").innerHTML += newHTML
}
...
function search(){
    ...
    let recipeList = recipes.filter(recipe => {
        return (
            //search through each list
            recipe.name.includes(input) ||
            recipe.description.includes(input) ||
            recipe.tags.includes(input)
        )
    })
    displayRecipies(recipeList)
}
