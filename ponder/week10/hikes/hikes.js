...
function search() {
  let hikeQuerry = input.value.toLowerCase();

  let filteredHikes = hikes.filter(function(hike) {
    return (
      hike.name.toLowerCase().includes(hikeQuerry) ||
      hike.description.toLowerCase().includes(hikeQuerry) ||
      hike.tags.find(tag => tag.toLowerCase().includes(hikeQuerry))
  );
...
let rndIndex = Math.floor(Math.random() * hikes.length);
console.log(rndIndex);
let rndHike = hikes[rndIndex];
